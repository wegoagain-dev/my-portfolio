---
title: How I Deployed a Python Chat App (Docker Setup) - Part 2
date: 2025-11-03
...
- [Part 1: Local Manual Setup →](/projects/chat-app-eks-part-1)
- **Part 2: Docker Setup** (You are here)
...
In [Part 1](/projects/chat-app-eks-part-1), we got our Python chat app running locally. While it worked, the process was a mess:
...
3. **Fragile & Not Portable:** The only reason it worked was because our app's default database host (`localhost`) happened to match our manual Redis install. This would break on any other machine.

Now, we will solve every one of these problems with **Docker**. We'll package our app and its database into isolated, portable "containers" that will run identically on any machine.
...
- **`COPY requirements.txt /` & `RUN pip install -r requirements.txt`** This is the **most important optimisation**. Notice we are not copying the whole app first. We only copy `requirements.txt` and then ran `pip install`. This creates a **cached layer**. When we change our Python code (`app.py`) and rebuild, Docker sees that `requirements.txt` hasn't changed, so it **skips** this entire slow `pip install` step and uses a cached version. This makes our builds incredibly fast.
...
Now we'll run the exact same two-component system as Part 1, but this time, everything will be in containers.
...
If you have been watching and learning about Docker, you will now understand that docker images are built using Dockerfiles and in our directory we have that Dockerfile. We will now build the image, then run it to stick with the redis container running in that `chat-net` network.
...
This command looks for the Dockerfile in the directory we are in and uses it to create our image.

`-t my-chat-app` is a tag which gives our application a name otherwise itll give us some random cryptic ID, and `.` says where the applications source code will be (which is the current directory)
...
- `--network chat-net`: Connects our app to the same network as the Redis container.
...
## Congrats!
Now, you can open your browser to http://localhost:8080, and the app works perfectly. This is because we exposed the container's port 8080 to our host machine's port 8080 using the `-p` flag, otherwise we would not be able to access the app from our host machine.
...
**In our next post** [Part 3](/projects/chat-app-eks-part-3), we'll take this a step further. We'll build an automated CI pipeline with GitHub Actions so that every time we push new code, our `my-chat-app` image is built and pushed to a central registry (ECR) automatically.



---

## Deeper Explanation: "Why" and "How"
...
### Part 2: The App's Configuration

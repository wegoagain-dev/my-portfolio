---
title: How I Deployed a Python Chat App (Complete EKS Deployment) - Part 4
date: 2025-11-05
...
- **Part 4: Kubernetes Deployment (EKS)**(You are here)

## "Why are we doing this": We Need to Run Our App
Our journey is almost to completion (maybe we will build on this project in the future :D ). In [Part 3](/projects/chat-app-eks-part-3), we built an automated CI pipeline. Our `my-chat-app` image is now built and pushed to an AWS ECR "warehouse" every time we commit code.

But that's where it stops. Our image is just sitting in a warehouse, not running.

In this (maybe) final post, we'll build our "factory", a production-grade Amazon EKS (Kubernetes) Cluster and deploy our application for the world to see.
...
We will use this policy to create a policy profile in AWS IAM console.
...
I actually got an error because i needed an IAM OIDC profile (what i was talking about in the previous part) and this is like a way to enable secure access between Kubernetes and the AWS resources.
...
### 2b. Installing AWS Load Balancer Controller
...
Now lets install the aws load balancer controller from the helm charts we added
(this is what watches for any `ingress.yaml` file)

(make sure to change cluster name, VPC and region to your own)

the service account we are attaching is what has the aws load balancer policy permission added to it and thats what will be allowed to create it.

```yaml
helm install aws-load-balancer-controller eks/aws-load-balancer-controller \
  -n kube-system \
  --set clusterName=python-redis-chat-1 \
...
<video width="650" height="300" controls>
  <source src="/demo-python-redis-app.mp4" type="video/mp4">

---
title: 'Part 3: CI/CD with GitHub Actions'
description: 'Setting up a CI/CD pipeline with GitHub Actions and AWS ECR to automatically build and push Docker images.'
date: 2025-11-04
order: 3
tags: ['GitHub Actions', 'AWS']
---

Similar to Part 2, I'd recommend having some familiarity with CI/CD pipelines, GitHub Actions, and AWS ECR before proceeding. A basic understanding of AWS in general will help.

In [Part 2](/blog/chat-app-eks/part-2), we created a perfect, robust Docker setup. Our app now runs reliably on our own laptop. But this still leaves two big problems:

**It's Manual**: If I change one line of code in app.py, I have to remember to manually run `docker build -t my-chat-app .` again.

**It's Local**: The image `my-chat-app` only exists on my machine. How do my teammates get it? How would a live server get it?

We need to fix this by building an automated **Continuous Integration/Continuous Delivery (CI/CD)** pipeline.
...
**Grant Permissions**: On next, select `Attach policies directly` and search for `AmazonEC2ContainerRegistryFullAccess`. This gives it just enough permission to log in and push images, and nothing else. (This is known as the **_principle of least privilege._**). Then next and create user.
...
To summarise: the IAM user is _who_ is doing something, the policy is _what_ it can do, and the access keys are _how_ it logs in.
...
In the final part of this series, we'll build the 'factory' to run it. We'll use `eksctl` to provision a live EKS (Kubernetes) cluster, then write the Kubernetes manifests to deploy our chat app and its Redis database from ECR to the live, public internet.

Let's head to [Part 4](/blog/chat-app-eks/part-4)

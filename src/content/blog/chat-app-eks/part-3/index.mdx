...
description: "Setting up a CI pipeline with GitHub Actions and AWS ECR to automatically build and push Docker images."
date: 2025-11-04
order: 3
tags: ['GitHub Actions', 'AWS', 'ECR', 'Docker']
authors: ['tawfiq']
---
...
To summarise: the IAM user is *who* is doing something, the policy is *what* it can do, and the access keys are *how* it logs in.
...
In the final part of this series, we'll build the 'factory' to run it. We'll use `eksctl` to provision a production-grade EKS (Kubernetes) cluster, then write the Kubernetes manifests to deploy our chat app and its Redis database from ECR to the live, public internet.

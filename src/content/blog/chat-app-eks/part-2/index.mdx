---
title: 'Part 2: Docker Containerisation'
description: 'Containerising our Python and Redis app, and connecting to two microservices using a Docker network.'
date: 2025-11-03
order: 2
tags: ['Docker', 'Python', 'Redis']
authors: ['tawfiq']
---

Before we dive in, I'd recommend watching a few tutorials on Docker if you're not familiar with it. Here are some good starting points:

1. [Docker Official Tutorial](https://docs.docker.com/get-started/)
2. [YouTube: Docker in 100 Seconds](https://youtu.be/pg19Z8LL06w?si=IJ_dusCFp6kaJfq5)
3. [YouTube: Docker Tutorial for Beginners](https://youtu.be/GFgJkfScVNU?si=7DMab3wxxmCD_IrR)

## The "Why": Solving Our Manual Mess
In [Part 1](/blog/chat-app-eks/part-1), we got our Python chat app running locally. While it worked, the process was a mess:
...
- **`FROM python:3.11`** This is our "fix" for the `greenlet` error. It doesn't matter that my Mac is running Python 3.13. The container will be built using a clean, official **Python 3.11** image, guaranteeing a stable and identical environment every time.
...
- **`ENTRYPOINT`** Instead of the typical `CMD`, we use `ENTRYPOINT` with our script to:
  ...
  2. **Data Initialisation**: Ensures demo data is properly loaded into Redis
  3. **Proper Startup**: Configures and starts gunicorn with the right settings

**What we learned from this file:**

1. The "it works on my machine" problem is solved by using `python:3.11`
2. Builds will be fast because of layer caching
3. The app runs on **port 8080** inside the container
...
The `--name redis-db` flag gives this container a name, which becomes its hostname on the `chat-net` network. The `--network chat-net` flag attaches it to our private network.
...
1. `-p 8080:8080` connects our host machine's port 8080 to the container's port 8080 (where `gunicorn` is running)
2. `--network chat-net` connects our app to the same network as the Redis container
3. `-e REDIS_ENDPOINT_URL=redis-db:6379` is the key part. We're setting the environment variable that tells the app where to find Redis. The value `redis-db:6379` specifies both the hostname and port. Every app is different and may need additional environment variables; this app only needs to know where Redis is.

(I'll explain in more detail how these connect at the bottom of this post)
...
1. **No Dependency Hell:** The app is built in a clean Python 3.11 environment. The `greenlet` error is gone. (The image just exists so you can just run the container and redis and it just works)
...
**In our next post** [Part 3](/blog/chat-app-eks/part-3), we'll take this a step further. We'll build an automated CI pipeline with GitHub Actions so that every time we push new code, our `my-chat-app` image is built and pushed to a central registry (ECR) automatically.
